                             ...
                             int direction)
{
  ...
  /*ret = mcp23x17_setbit(priv, MCP23X17_REG_CONFIG, pin,
  ...
  if (opt == IOEXPANDER_OPTION_INVERT)
    {
      ...
      /*ret = mcp23x17_setbit(priv, MCP23X17_REG_POLINV, pin, ival);*/
      mcp23x17_unlock(priv);
    }
                            ...
                            bool value)
{
  ...
  /*ret = mcp23x17_setbit(priv, MCP23X17_REG_OUTPUT, pin, value);*/
  mcp23x17_unlock(priv);
  return ret;
}
...
static int mcp23x17_getmultibits(FAR struct mcp23x17_dev_s *priv, uint8_t addr,
...
{
  FAR struct mcp23x17_dev_s *priv = (FAR struct mcp23x17_dev_s *)dev;
  uint8_t addr;/* = MCP23X17_REG_OUTPUT;*/
  uint8_t buf[3];
  int ret;
  int i;
  int index;
                                ...
                                int count)
{
  ...
  /*ret = mcp23x17_getmultibits(priv, MCP23X17_REG_INPUT,
                             pins, values, count);*/
  mcp23x17_unlock(priv);
  return ret;
}
                                ...
                                int count)
{
  ...
  /*ret = mcp23x17_getmultibits(priv, MCP23X17_REG_OUTPUT,
                             pins, values, count);*/
  mcp23x17_unlock(priv);
  return ret;
}
...
static int mcp23x17_detach(FAR struct ioexpander_dev_s *dev, FAR void *handle)
{
  FAR struct mcp23x17_dev_s *priv = (FAR struct mcp23x17_dev_s *)dev;
...
static void mcp23x17_irqworker(void *arg)
{
  ...
  uint8_t addr;/* = MCP23X17_REG_INPUT;*/
  uint8_t buf[2];
  ioe_pinset_t pinset;
  int ret;
  int i;
 ...
 *
 * Description:
 *   Set the initial configuration of the MCP23x17
 *
 * Input Parameters:
 *   dev - Device-specific state data
 *
...
static int mcp23x17_setup(FAR struct mcp23x17_dev_s *priv)
{
  uint8_t buf[2];
  int ret = -1;
                              ...
                              FAR struct mcp23x17_config_s *config)
{
  ...
  /* Do the initial configuration setup */

  ret = mcp23x17_setup(priv);
  if (ret < 0)
    {
      _err("Failed to do the initial setup\n");
    }
