...
/* Let's use IOCON.BANK = 0 Addressing Mode */
...
#define MCP23X17_TW                      1

/* 1us (datasheet: time to reset (Treset) is 600ns */

#define MCP23X17_TRESET                  1

#define MCP23X17_IRQ_TYPE_EDGE_BOTH      0x00000000
#define MCP23X17_IRQ_TYPE_EDGE_RISING    0x00000001
#define MCP23X17_IRQ_TYPE_EDGE_FALLING   0x00000002
#define MCP23X17_IRQ_TYPE_LEVEL_HIGH     0x00000001
#define MCP23X17_IRQ_TYPE_LEVEL_LOW      0x00000002

#define MCP23X17_IRQ_TYPE_EDGE           0x00000000
#define MCP23X17_IRQ_TYPE_LEVEL          0x00000001

#define MCP23X17_POLLDELAY       (CONFIG_MCP23X17_INT_POLLDELAY / USEC_PER_TICK)

#define MCP23X17_LEVEL_SENSITIVE(d,p) \
  (((d)->trigger  & ((ioe_pinset_t)1 << (p))) == 0)
#define MCP23X17_LEVEL_HIGH(d,p) \
  (((d)->level[0] & ((ioe_pinset_t)1 << (p))) != 0)
#define MCP23X17_LEVEL_LOW(d,p) \
  (((d)->level[1] & ((ioe_pinset_t)1 << (p))) != 0)

#define MCP23X17_EDGE_SENSITIVE(d,p) \
  (((d)->trigger  & ((ioe_pinset_t)1 << (p))) != 0)
#define MCP23X17_EDGE_RISING(d,p) \
  (((d)->level[0] & ((ioe_pinset_t)1 << (p))) != 0)
#define MCP23X17_EDGE_FALLING(d,p) \
  (((d)->level[1] & ((ioe_pinset_t)1 << (p))) != 0)
#define MCP23X17_EDGE_BOTH(d,p) \
  (MCP23X17_LEVEL_RISING(d,p) && MCP23X17_LEVEL_FALLING(d,p))
