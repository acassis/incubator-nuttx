...
/* Just use max. 4MB SPIRAM for now */
...
void __attribute__((constructor)) esp_himem_init(void)
{
  ...
  HIMEM_CHECK(s_ram_descriptor != NULL, "already initialized",);
  ...
  s_range_descriptor = calloc(sizeof(rangeblock_t),
                              SPIRAM_BANKSWITCH_RESERVE);

  if (s_ram_descriptor == NULL || s_range_descriptor == NULL)
    {
      _err(TAG, "Cannot allocate memory for meta info. Not initializing!");
      free(s_ram_descriptor);
      free(s_range_descriptor);
      return;
    }

  _info(TAG, "Initialized. Using last %d 32KB address blocks for bank\
...
static bool allocate_blocks(int count, uint16_t *blocks_out)
{
  ...
  for (i = 0; i < s_ramblockcnt && n != count; i++)
    {
      if (!s_ram_descriptor[i].is_alloced)
        {
          blocks_out[n] = i;
  ...
  for (i = 0; i < handle->block_ct; i++)
    {
      ...
      HIMEM_CHECK(s_ram_descriptor[handle->block[i]].is_mapped,
                  "block in range still mapped", ESP_ERR_INVALID_ARG);
    }
...
esp_err_t esp_himem_alloc_map_range(size_t size,
                                    esp_himem_rangehandle_t *handle_out)
{
  ...
  HIMEM_CHECK(s_ram_descriptor == NULL, "Himem not available!",
  ...
  for (i = 0; i < handle->block_ct; i++)
    {
      ...
      HIMEM_CHECK(s_range_descriptor[i + handle->block_start].is_mapped,
                  "memory still mapped to range", ESP_ERR_INVALID_ARG);
    }
...
esp_err_t esp_himem_map(esp_himem_handle_t handle,
                        ...
                        void **out_ptr)
{
  ...
  HIMEM_CHECK(ram_offset % CACHE_BLOCKSIZE != 0,
              "ram offset not aligned to blocksize", ESP_ERR_INVALID_ARG);

  HIMEM_CHECK(range_offset % CACHE_BLOCKSIZE != 0,
              "range not aligned to blocksize", ESP_ERR_INVALID_ARG);

  HIMEM_CHECK(len % CACHE_BLOCKSIZE != 0,
              "length not aligned to blocksize", ESP_ERR_INVALID_ARG);

  /* ram and range should be within allocated range */

  HIMEM_CHECK(ram_block + blockcount > handle->block_ct,
              "args not in range of phys ram handle", ESP_ERR_INVALID_SIZE);

  HIMEM_CHECK(range_block + blockcount > range->block_ct,
              "args not in range of range handle", ESP_ERR_INVALID_SIZE);
  ...
  for (i = 0; i < blockcount; i++)
    {
      HIMEM_CHECK(s_ram_descriptor[handle->block[i + ram_block]].is_mapped,
                  "ram already mapped", ESP_ERR_INVALID_STATE);

      HIMEM_CHECK(s_range_descriptor[range->block_start + i +
  ...
  for (i = 0; i < blockcount; i++)
    {
      ...
      s_range_descriptor[range->block_start + i + range_block].ram_block =
...
esp_err_t esp_himem_unmap(esp_himem_rangehandle_t range, void *ptr,
                          size_t len)
{
  ...
  HIMEM_CHECK(range_offset % CACHE_BLOCKSIZE != 0,
              "range offset not block-aligned", ESP_ERR_INVALID_ARG);

  HIMEM_CHECK(len % CACHE_BLOCKSIZE != 0,
              "map length not block-aligned", ESP_ERR_INVALID_ARG);

  HIMEM_CHECK(range_block + blockcount > range->block_ct,
              "range out of bounds for handle", ESP_ERR_INVALID_ARG);

  spinlock_flags = spin_lock_irqsave();

  for (i = 0; i < blockcount; i++)
    {
      int ramblock = s_range_descriptor[range->block_start + i +
