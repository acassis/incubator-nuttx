...
static int adapter_coex_wifi_channel_get(uint8_t *primary, uint8_t *secondary);

static void adapter_coex_schm_status_bit_clear(uint32_t type, uint32_t status);
...
int coex_bt_request_wrapper(uint32_t event, uint32_t latency, uint32_t duration);
...
static struct osi_funcs_s g_osi_funcs =
{
  ...
  ._coex_register_wifi_channel_change_callback = adapter_coex_register_wifi_channel_change_callback,
  ...
  ._coex_schm_register_btdm_callback = adapter_coex_schm_register_btdm_callback,
...
static void bt_periph_module_enable(periph_module_t periph)
{
...
int esp32_bt_controller_init(void)
{
  ...
  wlinfo("BT controller compile version [%s]", btdm_controller_get_compile_version());
...
int esp32_bt_controller_init(void)
{
  ...
  if (esp32_rtc_clk_slow_freq_get() == RTC_SLOW_FREQ_32K_XTAL)
    {
  ...
  if (g_lp_cntl.lpclk_sel == BTDM_LPCLK_SEL_XTAL)
    {
      ...
      set_div_ret = btdm_lpclk_set_div(esp32_rtc_clk_xtal_freq_get() * 2 - 1);
  ...
  modifyreg32(DPORT_WIFI_CLK_EN_REG, 0, UINT32_MAX);
