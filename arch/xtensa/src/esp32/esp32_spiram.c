...
/* Simple RAM test. Writes a word every 32 bytes. Takes about a second to
 * complete for 4MiB. Returns true when RAM seems OK, false when test fails.
 * WARNING: Do not run this before the 2nd cpu has been initialized (in a
 * two-core system) or after the heap allocator has taken ownership of the
 * memory.
 */
...
bool esp_spiram_test(void)
{
  ...
  if (errct)
    {
      _err("SPI SRAM memory test fail. %d/%d writes failed,\
           first @ %X\n", errct, s / 32,
           initial_err + SOC_EXTRAM_DATA_LOW);

      return false;
    }
  else
    {
      _info("SPI SRAM memory test OK");
      return true;
    }
}
...
esp_err_t esp_spiram_init(void)
{
  ...
  if (r != ESP_OK)
    {
#ifdef CONFIG_SPIRAM_IGNORE_NOTFOUND
      _err("SPI RAM enabled but initialization failed.\
           Bailing out.");
#endif
      return r;
    }
  ...
  if (esp_spiram_get_size() != CONFIG_SPIRAM_SIZE)
    {
      _err("Expected %dKiB chip but found %dKiB chip.\
           Bailing out..", CONFIG_SPIRAM_SIZE / 1024,
           esp_spiram_get_size() / 1024);
      return ESP_ERR_INVALID_SIZE;
    }
#endif

  _info("Found %dMBit SPI RAM device",
        (esp_spiram_get_size() * 8) / (1024 * 1024));

  _info("SPI RAM mode: %s",
  ...
  _info("PSRAM initialized, cache is in %s mode.", \
...
{
  /* Add entire external RAM region to heap allocator.
   * Heap allocator knows the capabilities of this type of memory,
   * so there's no need to explicitly specify them.
   */

#ifdef CONFIG_BOOT_SDRAM_DATA
    _info("Adding pool of %dK of external SPI memory to heap allocator",
          (spiram_size_usable_for_malloc() -
           (&_ext_ram_bss_end - &_ext_ram_bss_start)) / 1024);

  return heap_caps_add_region((intptr_t)&_ext_ram_bss_end,
                              (intptr_t)SOC_EXTRAM_DATA_LOW +
                               spiram_size_usable_for_malloc() - 1);
#else
  _info("Adding pool of %dK of external SPI memory to heap allocator",
        spiram_size_usable_for_malloc() / 1024);

  return umm_addregion((FAR void *)SOC_EXTRAM_DATA_LOW,
                       spiram_size_usable_for_malloc() - 1);
#endif
}
...
esp_err_t esp_spiram_reserve_dma_pool(size_t size)
{
...
#if 0
  _info("Reserving pool of %dK of internal memory for DMA/internal\
  ...
  while (size > 0)
    {
      ...
      _info("Allocating block of size %d bytes", next_size);
      ...
      esp_err_t e = heap_caps_add_region_with_caps(caps, (intptr_t) dma_heap,
  ...
  return ESP_OK;
#endif
  return ESP_ERR_NOT_SUPPORTED; /* DMA not supported */
}
...
void IRAM_ATTR esp_spiram_writeback_cache(void)
{
  ...
  if (DPORT_REG_GET_BIT(DPORT_PRO_CACHE_CTRL_REG,
                        DPORT_PRO_CACHE_ENABLE) == 0)
    {
      cache_was_disabled |= (1 << 0);
      DPORT_SET_PERI_REG_BITS(DPORT_PRO_CACHE_CTRL_REG, 1, 1,
                              DPORT_PRO_CACHE_ENABLE_S);
    }

#ifdef CONFIG_SMP
  if (DPORT_REG_GET_BIT(DPORT_APP_CACHE_CTRL_REG,
                        DPORT_APP_CACHE_ENABLE) == 0)
    {
      cache_was_disabled |= (1 << 1);
      DPORT_SET_PERI_REG_BITS(DPORT_APP_CACHE_CTRL_REG, 1, 1,
  ...
  if (cache_was_disabled & (1 << 0))
    {
      while (DPORT_GET_PERI_REG_BITS2(DPORT_PRO_DCACHE_DBUG0_REG,
             DPORT_PRO_CACHE_STATE, DPORT_PRO_CACHE_STATE_S) != 1)
        {
        };

      DPORT_SET_PERI_REG_BITS(DPORT_PRO_CACHE_CTRL_REG, 1, 0,
                                DPORT_PRO_CACHE_ENABLE_S);
    }
  ...
  if (cache_was_disabled & (1 << 1))
    {
      while (DPORT_GET_PERI_REG_BITS2(DPORT_APP_DCACHE_DBUG0_REG,
             DPORT_APP_CACHE_STATE, DPORT_APP_CACHE_STATE_S) != 1)
        {
        };

      DPORT_SET_PERI_REG_BITS(DPORT_APP_CACHE_CTRL_REG, 1, 0,
                                DPORT_APP_CACHE_ENABLE_S);
    }
#endif
}
