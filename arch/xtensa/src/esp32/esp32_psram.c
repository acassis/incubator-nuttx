                     ...
                     psram_cmd_mode_t cmd_mode)
{
  ...
  uint32_t mode_backup = (READ_PERI_REG(SPI_USER_REG(spi_num)) >>
                          SPI_FWRITE_DUAL_S) & 0xf;

  uint32_t rd_mode_backup = READ_PERI_REG(SPI_CTRL_REG(spi_num)) &
  ...
  if (data)
    {
      ...
      do
        {
          *data++ = READ_PERI_REG(SPI_W0_REG(spi_num) + (idx << 2));
        }
...
#if defined(CONFIG_SPIRAM_2T_MODE)
...
{
  /* Not needed because the bootloader already did it! */
}
...
psram_enable(psram_cache_mode_t mode, psram_vaddr_mode_t vaddrmode)   /* psram init */
{
  ...
  const uint32_t spiconfig;

  if (pkg_ver == EFUSE_RD_CHIP_VER_PKG_ESP32D2WDQ5)
    {
      _info("This chip is ESP32-D2WD");
      rtc_vddsdio_config_t cfg = rtc_vddsdio_get_config();
      if (cfg.tieh != RTC_VDDSDIO_TIEH_1_8V)
        {
          _err("VDDSDIO is not 1.8V");
          return ESP_FAIL;
        }
  ...
  else
    {
      if ((pkg_ver == EFUSE_RD_CHIP_VER_PKG_ESP32PICOD2) ||
        ...
        {
          _info("This chip is ESP32-PICO");
          rtc_vddsdio_config_t cfg = rtc_vddsdio_get_config();
          if (cfg.tieh != RTC_VDDSDIO_TIEH_3_3V)
            {
              _err("VDDSDIO is not 3.3V");
              return ESP_FAIL;
            }
  ...
  if (spiconfig == EFUSE_SPICONFIG_SPI_DEFAULTS)
    ...
    {
      if (spiconfig == EFUSE_SPICONFIG_HSPI_DEFAULTS)
        {
  ...
  if (psram_is_32mbit_ver0())
    {
      ...
      if (mode == PSRAM_CACHE_F80M_S80M)
        {
          ...
          _err("This version of PSRAM needs to claim an extra SPI"
	   ...
	   * clock (but not the reset!) to that peripheral.
          ...
          while (1)
	        ...
	        {
                  modifyreg32(DPORT_PERIP_CLK_EN_REG, PSRAM_SPICLKEN, 0);
                  break;
                }
            }
#endif
...
#  if CONFIG_SPIRAM_BANKSWITCH_ENABLE
      _err("PSRAM 2T mode and SPIRAM bank switching can not enabled\
      ...
      if (psram_2t_mode_check(PSRAM_SPI_1) != ESP_OK)
        {
          ...
          if (psram_2t_mode_check(PSRAM_SPI_1) != ESP_OK)
	    {
              _err(TAG, "PSRAM 2T mode enable fail!");
              return ESP_FAIL;
            }
        }

      s_2t_mode_enabled = true;
      _info(TAG, "PSRAM is in 2T mode");
#endif
    }
                 ...
                 psram_vaddr_mode_t vaddrmode)
{
  ...
  DPORT_SET_PERI_REG_BITS(DPORT_PRO_CACHE_CTRL1_REG,
                          DPORT_PRO_CMMU_SRAM_PAGE_MODE, 0,
  ...
  DPORT_SET_PERI_REG_BITS(DPORT_APP_CACHE_CTRL1_REG,
                          DPORT_APP_CMMU_SRAM_PAGE_MODE, 0,
