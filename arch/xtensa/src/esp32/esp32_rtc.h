 ...
 *
 * These functions are not thread safe, and should not be viewed as high
 * level APIs. For example, while this file provides a function which can
 * switch CPU frequency, this function is on its own is not sufficient to
 * implement frequency switching in ESP-IDF context: some coordination with
 * RTOS, peripheral drivers, and WiFi/BT stacks is also required.
 *
 ...
 * - rtc_time: reading RTC counter, conversion between counter values and
 *   time
 * - rtc_sleep: entry into sleep modes
 * - rtc_init: initialization
 */

/* Description: Possible main XTAL frequency values.
 *
...
/* Description: CPU frequency values */
...
/* Description: CPU clock configuration structure */
...
/* Description: RTC SLOW_CLK frequency values */

typedef enum
{
  RTC_SLOW_FREQ_RTC = 0,      /* Internal 150 kHz RC oscillator */
  RTC_SLOW_FREQ_32K_XTAL = 1, /* External 32 kHz XTAL */
  RTC_SLOW_FREQ_8MD256 = 2,   /* Internal 8 MHz RC divided by 256. */
} rtc_slow_freq_t;

/* Description: RTC FAST_CLK frequency values */
...
/* With the default value of CK8M_DFREQ, 8M clock is 8.5 MHz +/- 7% */

#define RTC_FAST_CLK_FREQ_APPROX 8500000

/* Description: Clock source to be calibrated using rtc_clk_cal function */

typedef enum
{
  RTC_CAL_RTC_MUX = 0,       /* Currently selected RTC SLOW_CLK */
  RTC_CAL_8MD256 = 1,        /* Internal 8 MHz RC osc, divided by 256 */
  RTC_CAL_32K_XTAL = 2       /* External 32 kHz XTAL */
} rtc_cal_sel_t;

/* Description: Initialization parameters for rtc_clk_init */
...
typedef struct rtc_clk_config_s
{
    ...
    uint32_t clk_8m_div : 3;            /* RTC 8M clock divider (division is
                                         * by clk_8m_div+1, i.e. 0 means 8MHz
                                         * frequency)
                                         */
    uint32_t slow_clk_dcap : 8;         /* RTC 150k clock adjustment
                                         * parameter (higher value leads
                                         * to lower frequency)
                                         */
    uint32_t clk_8m_dfreq : 8;          /* RTC 8m clock adjustment parameter
                                         * (higher value leads to higher
                                         * frequency)
                                         */
} rtc_clk_config_t;

/* Description: Default initializer for rtc_clk_config_t */
...
/* Description: Initialize clocks and set CPU frequency
 *
 ...
 * XTAL frequency with the frequency of internal 8MHz oscillator. Note that
 * at high temperatures the frequency of the internal 8MHz oscillator may
 * drift enough for auto detection to be unreliable.
 ...
 * If XTAL frequency can not be auto detected, this 26MHz frequency will be
 * used.
 *
 * param:
 *   cfg  clock configuration as rtc_clk_config_t
 */

void rtc_clk_init(rtc_clk_config_t cfg);

/* Description: Get main XTAL frequency
 *
 * This is the value stored in RTC register RTC_XTAL_FREQ_REG by the
 * bootloader. As passed to rtc_clk_init function, or if the value was
 * RTC_XTAL_FREQ_AUTO, the detected XTAL frequency.
 *
 * return:
 *   XTAL frequency, one of rtc_xtal_freq_t
 */

rtc_xtal_freq_t rtc_clk_xtal_freq_get(void);

/* Description: Update XTAL frequency
 *
 * Updates the XTAL value stored in RTC_XTAL_FREQ_REG. Usually this value is
 * ignored after startup.
 *
 * param:
 *   xtal_freq New frequency value
 */

void rtc_clk_xtal_freq_update(rtc_xtal_freq_t xtal_freq);

/* Description: Enable or disable 32 kHz XTAL oscillator
 * param:
...
/* Description: Configure 32 kHz XTAL oscillator to accept external clock
 * signal
 */

void rtc_clk_32k_enable_external(void);

/* Description: Get the state of 32k XTAL oscillator
 * return:
...
/* Description: Enable 32k oscillator, configuring it for fast startup time.
 ...
 *
 * param:
 *   cycle Number of 32kHz cycles to bootstrap external crystal.
 *   If 0, no square wave will be used to bootstrap crystal oscillation.
 */

void rtc_clk_32k_bootstrap(uint32_t cycle);

/* Description: Enable or disable 8 MHz internal oscillator
 *
 ...
 * 8md256 or simply d256. Divider values other than 256 may be configured,
 * but this facility is not currently needed, so is not exposed in the code.
 *
 * When 8MHz/256 divided output is not needed, the divider should be disabled
 * to reduce power consumption.
 *
 * params:
 *   clk_8m_en true to enable 8MHz generator
 *   d256_en   true to enable /256 divider
 */

void rtc_clk_8m_enable(bool clk_8m_en, bool d256_en);

/* Description: Get the state of 8 MHz internal oscillator
 * return:
 *   true if the oscillator is enabled
 */

bool rtc_clk_8m_enabled(void);

/* Description: Get the state of /256 divider which is applied to 8MHz clock
 * return:
 *   true if the divided output is enabled
 */

bool rtc_clk_8md256_enabled(void);

/* Description: Enable or disable APLL
 *
...
/* Description: Select source for RTC_SLOW_CLK
 * param:
 *   slow_freq clock source (one of rtc_slow_freq_t values)
 */

void rtc_clk_slow_freq_set(rtc_slow_freq_t slow_freq);

/* Description: Get the RTC_SLOW_CLK source
 * return:
 *   currently selected clock source (one of rtc_slow_freq_t values)
 */

rtc_slow_freq_t rtc_clk_slow_freq_get(void);

/* Description: Get the approximate frequency of RTC_SLOW_CLK, in Hz
 *
...
/* Description: Select source for RTC_FAST_CLK
 * param:
 *   fast_freq clock source (one of rtc_fast_freq_t values)
 */

void rtc_clk_fast_freq_set(rtc_fast_freq_t fast_freq);

/* Description: Get the RTC_FAST_CLK source
 * return:
 *   currently selected clock source (one of rtc_fast_freq_t values)
 */

rtc_fast_freq_t rtc_clk_fast_freq_get(void);

/* Description: Get CPU frequency config corresponding to a rtc_cpu_freq_t
 *   value
 * params:
 *   cpu_freq CPU frequency enumeration value
 *   [out]    out_config  Output, CPU frequency configuration structure
 */

void rtc_clk_cpu_freq_to_config(rtc_cpu_freq_t cpu_freq,
                                rtc_cpu_freq_config_t *out_config);

/* Description: Get CPU frequency config for a given frequency
 * params:
 *   freq_mhz  Frequency in MHz
 *   [out]     out_config Output, CPU frequency configuration structure
 *
 * return:
 *   true if frequency can be obtained, false otherwise
 */

bool rtc_clk_cpu_freq_mhz_to_config(uint32_t freq_mhz,
                                    rtc_cpu_freq_config_t *out_config);

/* Description: Switch CPU frequency
 *
 * This function sets CPU frequency according to the given configuration
 * structure. It enables PLLs, if necessary.
 *
 * param:
 *   config  CPU frequency configuration structure
 */

void rtc_clk_cpu_freq_set_config(const rtc_cpu_freq_config_t *config);

/* Description: Switch CPU frequency (optimized for speed)
 *
 * This function is a faster equivalent of rtc_clk_cpu_freq_set_config.
 * It works faster because it does not disable PLLs when switching from PLL]
 * to XTAL and does not enabled them when switching back. If PLL is not
 * already enabled when this function is called to switch from XTAL to PLL
 * frequency, or the PLL which is enabled is the wrong one, this function
 * will fall back to calling rtc_clk_cpu_freq_set_config.
 *
...
/* Description: Get the currently used CPU frequency configuration
 * param:
 *   [out] out_config  Output, CPU frequency configuration structure
 */
 ...
 * Assumes that XTAL frequency has been determined — don't call in
 * startup
 * code.
 */
...
/* Description: Store new APB frequency value into RTC_APB_FREQ_REG
 *
 ...
 * RTC_APB_FREQ_REG (one of RTC general purpose retention registers).
...
/* Description: Get the current stored APB frequency.
 * return:
 *   The APB frequency value as last set via rtc_clk_apb_freq_update(),
 *   in Hz.
 */

uint32_t rtc_clk_apb_freq_get(void);

#define RTC_CLK_CAL_FRACT  19  /* Number of fractional bits in values
                                * returned by rtc_clk_cal.
                                */

/* Description: Measure RTC slow clock's period, based on main XTAL frequency
 *
 * This function will time out and return 0 if the time for the given number
 * of cycles to be counted exceeds the expected time twice. This may happen
 * if 32k XTAL is being calibrated, but the oscillator has not started up
 * (due to incorrect loading capacitance, board design issue, or lack of
 * 32KHz XTAL on board).
 *
 * params:
 *   cal_clk          clock to be measured
 *   slow_clk_cycles  number of slow clock cycles to average
 *   average          slow clock period in microseconds, Q13.19 fixed point
 *                    format, or 0 if calibration has timed out
 */

uint32_t rtc_clk_cal(rtc_cal_sel_t cal_clk, uint32_t slow_clk_cycles);

/* Description: Measure ratio between XTAL frequency and RTC slow clock
 * frequency
 * params:
 *   cal_clk         slow clock to be measured
 *   slow_clk_cycles number of slow clock cycles to average
 *   average         ratio between XTAL frequency and slow clock frequency,
 *                   Q13.19 fixed point format, or 0 if calibration has timed
 *                   out.
 */

uint32_t rtc_clk_cal_ratio(rtc_cal_sel_t cal_clk, uint32_t slow_clk_cycles);

/* Description: Convert time interval from microseconds to RTC_SLOW_CLK
 *              cycles
 * params:
 *   time_in_us       Time interval in microseconds
 *   slow_clk_period  Period of slow clock in microseconds, Q13.19
...
/* Description: Convert time interval from RTC_SLOW_CLK to microseconds
 * params:
...
/* Description: Get current value of RTC counter
 *
...
/* Description: Busy loop until next RTC_SLOW_CLK cycle
 *
...
/* Description: sleep configuration for rtc_sleep_init function */
...
typedef struct rtc_sleep_config_s
{
  ...
  uint32_t rtc_mem_inf_fpu : 1;       /* force normal voltage in sleep mode
                                       * (RTC memory)
                                       */
  ...
  uint32_t dig_dbias_slp : 3;          /* set bias for digital domain, in sleep
                                        * mode
                                        */
  ...
  uint32_t rtc_dbias_slp : 3;          /* set bias for RTC domain, in sleep
                                        * mode
                                        */
  uint32_t lslp_meminf_pd : 1;         /* remove all peripheral force power
                                        * up flags
                                        */
...
/* Description: Default initializer for rtc_sleep_config_t
 *
 * This initializer sets all fields to "reasonable" values (e.g. suggested
 * for production use) based on a combination of RTC_SLEEP_PD_x flags.
 *
 * param:
 * RTC_SLEEP_PD_x flags combined using bitwise OR
 */
...
/* Description: Prepare the chip to enter sleep mode
 *
 ...
 * wakeup sources.
 * params:
 *   cfg sleep mode configuration
 */

void rtc_sleep_init(rtc_sleep_config_t cfg);

/* Description: Set target value of RTC counter for RTC_TIMER_TRIG_EN wakeup
 * source
 * params:
...
/* Description: Enter deep or light sleep mode
 *
 ...
 * returns. When the chip wakes up from deep sleep, CPU is reset and
 * execution starts from ROM bootloader.
 *
 ...
 *
 * params:
 *   wakeup_opt  bit mask wake up reasons to enable (RTC_xxx_TRIG_EN flags
 *               combined with OR)
...
/* Description: Default initializer of rtc_config_t.
 *
 * This initializer sets all fields to "reasonable" values (e.g. suggested
 * for production use).
 */
...
/* Description: Initialize RTC clock and power control related functions
 * param:
 *   cfg configuration options as rtc_config_t
 */
...
/* Description: Structure describing vddsdio configuration */
...
/* Description: Get current VDDSDIO configuration
 ...
 * return:
 *   currently used VDDSDIO configuration
 */

rtc_vddsdio_config_t rtc_vddsdio_get_config(void);

/* Description: Set new VDDSDIO configuration using RTC registers.
 * If config.force == 1, this overrides configuration done using
 * bootstrapping pins and EFUSE.
 *
 * param:
